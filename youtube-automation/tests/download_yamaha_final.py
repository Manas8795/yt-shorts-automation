...
# Click on the video card on the canvas
...
dl_btn = page.locator("button[aria-label='Download scene'], button[aria-label*='Download'], button:has-text('download')").first
print(f"Clicking Download button: {dl_btn.get_attribute('aria-label')}...")
...
try:
    ...
    print(f"Downloaded video saved to: {target_video_path}")
except Exception as e:
    print(f"Expect download timeout: {e}. Checking video src / fetch fallback...")
    # Fallback: Extract video element source
    video_src = page.evaluate("""() => {
        const vid = document.querySelector('video');
        return vid ? vid.currentSrc || vid.src : null;
    }""")
    ...
    if video_src and video_src.startswith("http"):
        ...
        print(f"Successfully downloaded direct video stream to {target_video_path}")
...
if is_valid:
    ...
    print("Verification failed: downloaded file was empty or missing.")
